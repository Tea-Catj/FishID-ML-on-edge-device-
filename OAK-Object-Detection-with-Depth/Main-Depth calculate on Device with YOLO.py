...
def estimate_fish_mass(length_mm, fish_species="unknown"):
    a = 0.0 # Placeholder
    b = 0.0 # Placeholders
    # --- REPLACE WITH REAL SPECIES-SPECIFIC VALUES ---
    # You MUST research and find appropriate 'a' and 'b' values for your fish species.
    # Example for a hypothetical common carp in Laos (values are illustrative, not actual):
    if fish_species == "common_carp":
        a = 0.0075 # Example: for W in g, L in cm
        b = 3.05   # Example
    elif fish_species == "tilapia":
        a = 0.012  # Example: for W in g, L in cm
        b = 2.9    # Example
    # Add more species as needed with their specific a and b values
    else:
        # Fallback to a very general (and less accurate) rule, e.g., if b=3 (isometric)
        # You'd typically want to avoid this without a specific 'a' for a species.
        print(f"Warning: No specific length-weight coefficients for {fish_species}. Using general approximation, which may be inaccurate.")
        # For a rough general fish:
        a = 0.01 # Very generic, assumes W in g, L in cm
        b = 3.0  # Isometric growth

    # Convert length to the unit used by your 'a' and 'b' coefficients (e.g., cm)
    # Calculate mass in grams first, then convert to kg
    estimated_mass_g = a * ((length_mm/10) ** b)
    return estimated_mass_g
...
if __name__ == '__main__':
    ...
    #the code commented below is for experiment
    ...
    with dai.Device(pipeline) as device:
        ...
        while True:
            ...
            for detection in detections:
                ...
                roi = roiData.roi
                roi = roi.denormalize(disparity3.shape[1], disparity3.shape[0])
                ...
                intrinsicsR = calibData.getCameraIntrinsics(dai.CameraBoardSocket.CAM_C, 640, 400) #640x400 is resolution chosen for mono cam
                print('Right mono camera focal length in pixels in calibration:', intrinsicsR[0][0])
                focal_length = intrinsicsR[0][0]

                #calculate real height and width of that bounding box
                width_real = (pixel_width*detection.spatialCoordinates.z)/focal_length
                height_real = (pixel_height*detection.spatialCoordinates.z)/focal_length
                ...
                # roi1_x = int((x1 + (x2-x1)*0.02)) /width  #  5% from left
                # roi1_y = int(y1 + (y2 - y1) / 2) /height # half left
                # roi2_x = int((x2 + (-x2 + x1)*0.02)) / width  # 10% from right
                # roi2_y = int(y1 + (y2 - y1) / 2) / height  # half  right
                ...
                # i_roi1_x = int(n_roi1_x*width)
                # i_roi1_y = int(n_roi1_y*height)
                # i_roi1_x_min = int(n_roi1_x_min*width)
                # i_roi1_y_min = int((n_roi1_y_min*height))

                # i_roi2_x = int(n_roi2_x*width)
                # i_roi2_y = int(n_roi2_y*height)
                # i_roi2_x_min = int(n_roi2_x_min*width)
                # i_roi2_y_min = int(n_roi2_y_min*height)
                ...
                #estimate fish mass 
                fish_mass = estimate_fish_mass(width_real, fish_species=label)
                ...
                cv2.putText(color2, f"length: {"{:.2f}".format(float(width_real/1000))} m", (x1 + 10, y1 + 100), cv2.FONT_HERSHEY_TRIPLEX, 0.5, 255)
                cv2.putText(color2, f"width: {"{:.2f}".format(float(height_real/1000))} m", (x1 + 10, y1 + 120), cv2.FONT_HERSHEY_TRIPLEX, 0.5, 255)
                cv2.putText(color2, f"weight: {"{:.2f}".format(float(fish_mass))} g", (x1 + 10, y1 + 140), cv2.FONT_HERSHEY_TRIPLEX, 0.5, 255)
