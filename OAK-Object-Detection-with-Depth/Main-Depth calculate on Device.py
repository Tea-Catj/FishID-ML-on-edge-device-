...
if __name__ == '__main__':
    ...
    class HostSync:
        ...
        def add_msg(self, name, msg):
            ...
            if len(synced) == 6:  # color, left, right, depth, nn
                # Remove old msgs
                for name, arr in self.arrays.items():
                    for i, obj in enumerate(arr):
    ...
    with dai.Device(pipeline) as device:
        ...
        qs.append(device.getOutputQueue(name="rectifiedLeft", maxSize=4, blocking=False))
        qs.append(device.getOutputQueue(name="rectifiedRight", maxSize=4, blocking=False))
        qs.append(device.getOutputQueue(name="disp"))
        qs.append(device.getOutputQueue(name="SpatialData", maxSize=4, blocking=False))
        ...
        while True:
            for q in qs:
                ...
                if msgs:    
                    ...
                    spatialData =  msgs['SpatialData'].getSpatialLocations()
                    for depthData in spatialData:
                        ...
                        cv2.putText(disparity3, f"X: {int(depthData.spatialCoordinates.x)/1000} m", (xmin + 10, ymin + 20), fontType, 0.5, color)
                        cv2.putText(disparity3, f"Y: {int(depthData.spatialCoordinates.y)/1000} m", (xmin + 10, ymin + 35), fontType, 0.5, color)
                        cv2.putText(disparity3, f"Z: {int(depthData.spatialCoordinates.z)/1000} m", (xmin + 10, ymin + 50), fontType, 0.5, color)
